...
public class CoreImprintTagGameTests {
    ...
    public void allFourImprintTagsLoadAndAreNonEmpty(GameTestHelper helper) {
        ...
        helper.succeedIf(() -> {
            if (soul == 0) {
                throw new AssertionError("heartstead:soul_imprintable is empty — no Soul Core could ever attune");
            }
            if (pastoral == 0) {
                throw new AssertionError("heartstead:pastoral_imprintable is empty");
            }
            if (verdant == 0) {
                throw new AssertionError("heartstead:verdant_imprintable is empty");
            }
            if (lithic == 0) {
                throw new AssertionError("heartstead:lithic_imprintable is empty");
            }
        });
    }
