 ...
 * {@link com.sigilstead.vault.VaultAccess} anyway, so a client claiming to be on tab 1 with a Pouch
 * gets nothing.
 */
public record VaultTabPayload(boolean anchorTab) implements CustomPacketPayload {

    public static final Type<VaultTabPayload> TYPE = new Type<>(Sigilstead.id("vault_tab"));
