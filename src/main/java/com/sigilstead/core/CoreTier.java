...
public enum CoreTier {
    ...
    public Component costDescription() {
        ...
        for (Cost cost : costs) {
            Component part = Component.translatable("item.sigilstead.core.tier_cost",
                    cost.count(), Component.translatable(cost.item().getDescriptionId()));
            joined = joined == null ? part : Component.translatable("item.sigilstead.core.tier_cost_join", joined, part);
        }
