...
import com.sigilstead.vault.VaultLayout;
...
public class VaultScreen extends AbstractContainerScreen<VaultMenu> {
    ...
    protected void init() {
        ...
        searchBox = addRenderableWidget(new EditBox(this.font,
                ...
                Component.translatable("gui.sigilstead.vault.search")));
        searchBox.setHint(Component.translatable("gui.sigilstead.vault.search"));
        ...
        activateButton = addRenderableWidget(Button.builder(
                        Component.translatable("gui.sigilstead.vault.activate"),
        ...
        for (int i = 0; i < kinds.length; i++) {
            ...
            confirmButtons[i] = addRenderableWidget(Button.builder(
                            Component.translatable("gui.sigilstead.vault.upgrade.confirm"),
    ...
    private static Component activationLabel(VaultStatePayload state) {
        if (state.activated()) {
            return Component.translatable("gui.sigilstead.vault.activated");
        }
        if (!state.everActivated()) {
            return Component.translatable("gui.sigilstead.vault.activate_free");
        }
        return Component.translatable("gui.sigilstead.vault.activate_cost", state.reactivationCost());
    }
    ...
    private static Component activationTooltip(VaultStatePayload state) {
        if (state.activated()) {
            return Component.translatable("gui.sigilstead.vault.activated.tip");
        }
        if (!state.everActivated()) {
            return Component.translatable("gui.sigilstead.vault.activate_free.tip");
        }
        if (state.canActivateFromVaultStock()) {
            return Component.translatable("gui.sigilstead.vault.activate_from_vault.tip", state.reactivationCost());
        }
        return Component.translatable("gui.sigilstead.vault.activate_cost.tip", state.reactivationCost());
    }
    ...
    private void extractAnchorTab(GuiGraphicsExtractor graphics) {
        ...
        graphics.text(this.font, Component.translatable("gui.sigilstead.vault.tab.anchor"),
                VaultLayout.GRID_X, 8, TEXT_COLOR, false);
        graphics.text(this.font, Component.translatable("gui.sigilstead.vault.capacity_summary",
                        state.distinctTypesUsed(), state.distinctTypeCap(), state.stackDepthCap()),
                VaultLayout.GRID_X, 24, TEXT_COLOR, false);
        graphics.text(this.font, Component.translatable("gui.sigilstead.vault.sigils_spent", state.sigilsSpent()),
    ...
    private enum SortMode {
        COUNT_DESC(Component.translatable("gui.sigilstead.vault.sort.count"),
        ...
        NAME_ASC(Component.translatable("gui.sigilstead.vault.sort.name"),
